...
sample_iso = load_model('pipeline  104.csv_iso.joblib').fit_transform(merged_data)
sample_llc = load_model('pipeline  50.csv_llc.joblib').fit_transform(merged_data)
...
if st.button('Predict'):
    if len(sample) > 0:
        lr_prediction = lr_model.predict(merged_data)
        dtr_prediction = dtr_model.predict(merged_data)
